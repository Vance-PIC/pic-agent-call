...
## 非 Windows Terminal 環境下的 `WT_SESSION` 模擬與 VS Code 多視窗隔離

在 **VS Code Terminal**、獨立 **PowerShell** 或 **CMD** 等非 Windows Terminal 環境下，系統預設不提供 `WT_SESSION` 環境變數。為解決此問題，專案提供了自動注入與隔離機制：

### 1. 一鍵安裝/更新腳本
同事可以直接在 PowerShell 中執行本專案提供的自動設定腳本：
```powershell
powershell -File ./scripts/setup-terminal-key.ps1
```

### 2. VS Code 多終端機 UUID 隔離原理
由於 VS Code 新增終端機時預設會自動繼承前一個終端機的環境變數，若僅在 `WT_SESSION` 不存在時才生成，會導致多個 VS Code 終端分頁共享相同的 UUID。

本專案的 `setup-terminal-key.ps1` 會在使用者 Profile 寫入以下過濾邏輯：
- **VS Code 終端機**：強制每次啟動時皆重新生成新的隨機 UUID，以防 VS Code 本身的環境變數繼承機制。
- **其他終端機**：僅在 `WT_SESSION` 不存在時補上。

這能 100% 解決在同一個 VS Code 視窗開啟多個 Terminal 註冊不同角色時，發生「互相覆蓋」的衝突。
